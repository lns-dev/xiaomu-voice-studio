...
test('distinguishes Debug, Alpha, Beta and formal Release builds', () => {
  ...
  assert.equal(detectBuildChannel({ isPackaged: true, version: '0.1.0-beta.1' }), 'beta');
  assert.equal(detectBuildChannel({ isPackaged: true, version: '1.0.0-rc.1' }), 'beta');
